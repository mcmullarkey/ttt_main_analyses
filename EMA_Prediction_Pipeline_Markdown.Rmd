...
ema_data_init <- read_csv("fried_mh_covid_data_ema.csv") %>% 
  clean_names() %>% 
  rename(
    stress_1 = q1, 
    stress_2 = q2, 
...
ema_data_sec <- ema_data_init %>% 
  filter(!id %in% too_few_days$id)

# Confirm they've been dropped

ema_data_sec %>% 
  filter(str_detect(id, "25351"))

## does anyone have more than 4 observations for any given day?
too_many_obs <- ema_data_sec %>% count(id, day) %>% filter(n > 4)
too_many_obs
...
# Confirming the later row was dropped for that participant and no one else has more than 4 observations in a day
...
base_model_rec_list <- list(rec_ema_umap, rec_ema_pca, rec_ema_raw_pred)
...
base_model_data_list <- list(train_data_with_summ_stats_imp_nzv, train_data_with_summ_stats_imp_nzv, train_data_with_summ_stats_imp_nzv)
...
set.seed(33)
folds_ema_pred <- vfold_cv(.y, v = 4, repeats = 10, strata = dep_deteriorate)
...
base_model_preds_all_rs <- map(base_model_fit_all_rs, ~{
  ...
  roc_auc(truth = dep_deteriorate, .estimate = .pred_TRUE)
